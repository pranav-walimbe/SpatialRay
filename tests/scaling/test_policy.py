...
def _context(name, *, target=1, queued=0.0, metrics=None):
    # build the public AutoscalingContext surface consumed by the adapter
    return SimpleNamespace(
        deployment_name=name,
        target_num_replicas=target,
        total_queued_requests=queued,
        aggregated_metrics=metrics,
        policy_state={},
    )


def test_ray_policy_reads_ingress_work_and_downstream_queue():
    """Ingress work rates and a pool queue produce a coordinated Ray target."""
    ...
    ingress = _context(
        ...
        metrics={
            ...
            request_rate_metric("inference"): {"a": 4.0, "b": 1.0},
        },
    )
    inference = _context("inference", target=1, queued=12.0)
...
def test_ray_policy_holds_when_ingress_metrics_are_absent():
    ...
    contexts = {
        ...
        "decode-id": _context("decode", target=4, queued=20.0),
    }
    decisions, _ = policy(contexts)
    assert decisions == {"ingress-id": 2, "decode-id": 4}


def test_ray_policy_weights_an_old_queue_with_retained_mean_work():
    """Queued work remains visible after the active arrival-rate window becomes empty."""
    policy = RayWorkloadAutoscalingPolicy(
        capacities={
            "decode": {
                "work_per_s": 100.0,
    ...
    ingress = _context(
        ...
        metrics={
            ...
            request_rate_metric("decode"): {"a": 0.0},
            mean_work_metric("decode"): {"a": 25.0},
        },
    )
    decode = _context("decode", queued=12.0)
