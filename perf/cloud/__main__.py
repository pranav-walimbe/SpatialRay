"""
Drives the disaggregated Serve load test on the connected Ray cluster and writes the text report.
"""
...
from perf.cloud.utils import save_text_report
...
def main() -> None:
    """Drive an n-request Poisson trace through the Serve graph and write the text report."""
    ...
    parser.add_argument("--out", default=None, help="destination path for the text report")
    ...
    out = Path(args.out) if args.out else _ASSETS_DIR / f"perf-{args.hardware}-{args.model}.txt"
    out.parent.mkdir(parents=True, exist_ok=True)
    save_text_report(report, out)
    print(f"wrote {out}")
